...
2. **Hugo Rebuild Necessity for Shared UI Elements**:
   - Modifying layout files under `hugo/layouts/` or `hugo/themes/<theme-id>/layouts/` (e.g., adding sidebar modals, changing `<nav>` logic) will NOT automatically reflect in the active browser, because PocketBase serves the static `/pb_public` directory.
