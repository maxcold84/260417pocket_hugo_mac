...
3. **Sort Parameter Constraints (JSVM + REST API)**:
   ...
   - **Rule:** For order/CMS lists that need newest-first display, fetch all pages without a system-field sort and sort the merged JavaScript array by `created` after fetching.
...
9. **Custom GET Route SSR Authentication & pb_auth Cookie JSON Gotcha**:
    ...
    - **Rule:** In PocketBase v0.36, `findAuthRecordByToken()` accepts token types such as `"auth"` as its variadic arguments, not collection models. Do not pass `findCollectionByNameOrId("_superusers")` as the second argument.
    - **Rule:** For CMS SSR routes, verify the signed auth token first, then assert that the resolved record belongs to `_superusers`. Do not use `parseUnverifiedJWT()` as an authorization fallback.
      ```javascript
      ...
      if (!authRecord || authRecord.collection().name !== "_superusers") return null;
      ```
